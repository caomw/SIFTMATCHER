...
int test() {
	...
	argv[0] = "SiftMatcher";
	argv[1] = "/storage/sdcard/matcher/list_keys.txt";
	argv[2] = "/storage/sdcard/matcher/matches.txt";
    ...
    if (argc < 3) {
        LOGI("Usage: ./SiftMatcher [options] list_keys.txt matches.txt\n");
        LOGI("Options are:\n\n");
        LOGI("-s [num]    Sequential matching, match current image to last [num] image\n");
        LOGI("-r          Recovery mode. Uses SiftMatcher.log to resume from last incomplete run of SiftMatcher.\n");
        LOGI("\n");
    ...
    if(argc - optind != 2) {
        LOGI("Missing input and output file\n");
        return -1;
    }
    ...
    if(max_seq == -1) {
        LOGI("[SiftMatcher] Using all images for matching\n");
    }
    else {
        LOGI("[SiftMatcher] Using a maximum of %d images for matching\n", max_seq);
    }
    ...
    if(recovery_mode) {
        ...
        if(fp == NULL) {
            ...
            LOGI("[SiftMatcher] Recovering from image %d matching to %d\n", recovered_i, recovered_j);
        }
    }
    ...
    if (f == NULL) {
		LOGI("Error opening file %s for reading\n", list_in);
...
//    num_keys = new int[num_images];
    ...
    */
    ...
    for (int i = recovered_i; i < num_images; i++) {
        ...
        char dir[1024];
		LOGI("%06d", i);
        sprintf(dir, "/storage/sdcard/matcher/%06d", i);

        path data_dir(dir);
        if(!is_directory(data_dir)) {
            LOGI("[SiftMatcher] Creating directory: %s\n", dir);
        ...
        for (int j = j_start; j < i; j++) {
            ...
            LOGI("[SiftMatcher] Matching image %d to %d for %d keys\n", i, j, num_keys_j);
            ...
            if(matches.size() >= 16) {
                sprintf(filename, "/storage/sdcard/matcher/%06d/SiftMatcher_%d_%d.txt", i, i, j);

                FILE *fp = fopen(filename, "w+");
                if(fp == NULL) {
					fprintf(stderr, "Can't open %s for writing\n", filename);
					LOGI("Can't open %s for writing\n", filename);
                    exit(-1);
                }
            ...
            FILE *fp = fopen("/storage/sdcard/matcher/SiftMatcher.log", "w+");
